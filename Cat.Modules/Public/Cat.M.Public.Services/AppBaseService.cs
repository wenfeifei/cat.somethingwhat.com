...
{
    public class AppBaseService<T> where T : Cat.M.Public.Models.BaseEntity
    {
        ...
        /// <param name="listOrderBy"></param>
        /// <returns></returns>
        protected Models.Response.PageResult GetByPage(int pn, int ps, List<T> list, IList<OrderBy> listOrderBy = null)
        {
            if (ps > 1000) ps = 1000;
            ...
            //var list = Entities.Where(expression).OrderBy(dicOrderBy).Skip((pn - 1) * ps).Take(ps).ToList();
            list = list.AsQueryable().OrderBy(dicOrderBy).Skip((pn - 1) * ps).Take(ps).ToList();
            //int count = Entities.Count(expression);
