...
namespace Cat.M.Cache.Services.MSCache
{
    public class MemoryCacheHelper
    {
        ...
        public bool Set(string key, object value, TimeSpan expiresSliding, TimeSpan expiressAbsoulte)
        {
            ...
            return Set(key, value, new MemoryCacheEntryOptions().SetSlidingExpiration(expiresSliding)
                    .SetAbsoluteExpiration(expiressAbsoulte));
        }
        ...
        /// <returns></returns>
        public bool Set(string key, object value, DateTime dateTime, bool isSliding = false)
        {
            TimeSpan expiresIn = dateTime - DateTime.Now;
        ...
        public bool Set(string key, object value, TimeSpan expiresIn, bool isSliding = false)
        {
            ...
            return Set(key, value, isSliding
                    ? new MemoryCacheEntryOptions().SetSlidingExpiration(expiresIn)
                    : new MemoryCacheEntryOptions().SetAbsoluteExpiration(expiresIn));
        }
        ...
        /// <returns></returns>
        private bool Set(string key, object value, MemoryCacheEntryOptions options)
        {
            Type t = value.GetType();
            if (t.BaseType.FullName == "System.Object")
            {
                // 这里对对象做序列化与反序列化，是为了防止缓存后的对象被修改
                var json = JsonConvert.SerializeObject(value);
                var obj = JsonConvert.DeserializeObject(json, t);
